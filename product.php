<?php
session_start();
require 'connection.php';

// 1. Navigation State Variable
$app_mode = $_SESSION['app_mode'] ?? 'buyer';

$id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

if (!$id) {
    header("Location: listings.php");
    exit;
}

$stmt = $pdo->prepare("SELECT p.*, u.user_name AS seller_name FROM products p JOIN users u ON p.seller_id = u.user_id WHERE p.product_id = ?");
$stmt->execute([$id]);
$product = $stmt->fetch();

if (!$product) {
    echo "<p style='text-align: center; margin-top: 50px;'>Product not found. <a href='listings.php'>Back to listings</a></p>";
    exit;
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
  <title>PopCart - <?= htmlspecialchars($product['product_name']) ?></title>
  <link rel="stylesheet" href="index.css"/>
  <script src="https://kit.fontawesome.com/af69bda2f2.js" crossorigin="anonymous"></script>
</head>
<body>
<header>
    <nav id="header">
      <a href="index.php">
        <img src="ChatGPT Image May 12, 2026, 07_33_09 PM.png" class="logo" alt="PopCart Logo" width="150" height="auto"/>
      </a>
      <div id="hamburger" onclick="toggleMenu()">
        <span></span>
        <span></span>
        <span></span>
      </div>
      <ul id="navMenu">
        <?php if(isset($_SESSION['user_id'])): ?>

          <?php if($app_mode === 'seller'): ?>
            <li><a href="seller-dashboard.php">Dashboard</a></li>
            <li><a href="my-listings.php">Inventory</a></li>
            <li><a href="my-orders.php">Fulfillment</a></li>
            <li>
              <a href="switch-mode.php?to=buyer" style="background: #3b82f6; color: white; padding: 8px 15px; border-radius: 20px; font-weight: bold;">
                <i class="fa-solid fa-cart-shopping"></i> Switch to Buying
              </a>
            </li>
            <li><a href="logout.php">Logout</a></li>

          <?php else: ?>
            <li><a href="index.php">Home</a></li>
            <li><a href="listings.php">Browse Listings</a></li>
            <li><a href="track-orders.php">Track Orders</a></li>
            <li>
              <a href="switch-mode.php?to=seller" style="background: #10b981; color: white; padding: 8px 15px; border-radius: 20px; font-weight: bold;">
                <i class="fa-solid fa-store"></i> Switch to Selling
              </a>
            </li>
            <li><a href="logout.php">Logout (<?= htmlspecialchars($_SESSION['user_name']) ?>)</a></li>
            <li><a href="cart.php"><i class="fa-solid fa-cart-arrow-down"></i></a></li>
          <?php endif; ?>

        <?php else: ?>
          <li><a href="index.php">Home</a></li>
          <li><a href="listings.php">Browse Listings</a></li>
          <li><a href="login.php" id="loginBtn">Login</a></li>
          <li><a href="register.php" id="registerBtn">Register</a></li>
          <li><a href="cart.php"><i class="fa-solid fa-cart-arrow-down"></i></a></li>
        <?php endif; ?>
      </ul>
    </nav>
  </header>

  <section id="productPage">

    <div id="imgBox">
      <?php if ($product['image']): ?>
        <img src="uploads/<?= htmlspecialchars($product['image']) ?>" alt="<?= htmlspecialchars($product['product_name']) ?>"/>
      <?php else: ?>
        <img src="Iphone 15.jpg" alt="No image available"/>
      <?php endif; ?>
    </div>

    <div id="productBox">

      <div class="productDetail">
        <h3 style="margin-top: 0;"><?= htmlspecialchars($product['product_name']) ?></h3>
        <p class="productPrice" style="color: #28a745; font-size: 24px; font-weight: bold;">R<?= number_format($product['price'], 2) ?></p>
        <p class="location" style="color: #666;"><i class="fa-solid fa-location-dot"></i> <?= htmlspecialchars($product['location']) ?></p>
      </div>

      <div class="productDetail">
        <p><strong>Category:</strong> <?= ucfirst(htmlspecialchars($product['category'])) ?></p>
      </div>

      <div class="productDetail">
        <p><strong>Description:</strong></p>
        <p style="line-height: 1.6; color: #444;"><?= nl2br(htmlspecialchars($product['description'])) ?></p>
      </div>

      <div class="productDetail">
        <p><strong>Seller:</strong> <?= htmlspecialchars($product['seller_name']) ?></p>
      </div>

      <div id="productBtns" style="display: flex; gap: 10px; margin-top: 20px;">
        <?php if (isset($_SESSION['user_id'])): ?>
          <?php if($app_mode === 'seller'): ?>
             <button class="cartbn" style="opacity: 0.5; cursor: not-allowed;" title="Switch to Buyer Mode to purchase">Switch to Buyer Mode to Add to Cart</button>
          <?php else: ?>
             <a href="cart.php?add=<?= $product['product_id'] ?>" style="flex: 1;">
               <button class="cartbn" style="width: 100%;"><i class="fa-solid fa-cart-plus"></i> Add to Cart</button>
             </a>
          <?php endif; ?>
        <?php else: ?>
          <a href="login.php" style="flex: 1;">
            <button class="cartbn" style="width: 100%;">Login to Add to Cart</button>
          </a>
        <?php endif; ?>

        <button class="sellerbn" style="flex: 1; background: #6c757d; color: white; border: none; border-radius: 4px; cursor: pointer;" onclick="document.getElementById('sellerContact').style.display='block'">
          <i class="fa-solid fa-envelope"></i> Contact Seller
        </button>
      </div>

      <div id="sellerContact" style="display:none; margin-top:15px; padding:15px; background:#f8f9fa; border-radius:8px; border: 1px solid #e9ecef;">
        <p style="margin: 0;">Contact seller: <strong><?= htmlspecialchars($product['seller_name']) ?></strong></p>
        <p style="font-size: 14px; color: #666; margin-top: 5px;">*Messaging functionality becomes available in your Tracking Dashboard after you place an order with this seller.</p>
      </div>

    </div>

  </section>

  <footer>
    <div class="col">
      <img src="ChatGPT Image May 12, 2026, 07_33_09 PM.png" class="footerLogo">
      <h4>Contact</h4>
      <p><strong>Address:</strong> 562 Wellington Road, Street 32, San Francisco</p>
      <p><strong>Phone:</strong> 0834425678</p>
      <p><strong>Hours:</strong> 10:00 - 18:00, Mon-Sat</p>
      <div class="follow">
        <h4>Follow us</h4>
        <div class="icon">
          <i class="fab fa-facebook-f"></i>
          <i class="fab fa-twitter"></i>
          <i class="fab fa-instagram"></i>
          <i class="fab fa-pinterest-p"></i>
          <i class="fab fa-youtube"></i>
        </div>
      </div>
    </div>
    <div class="col">
      <h4>About</h4>
      <a href="#">About us</a>
      <a href="#">Privacy Policy</a>
      <a href="#">Terms & Conditions</a>
    </div>
    <div class="col">
      <h4>My Account</h4>
      <?php if(isset($_SESSION['user_id'])): ?>
        <?php if($app_mode === 'seller'): ?>
          <a href="seller-dashboard.php">Dashboard</a>
          <a href="my-listings.php">Inventory</a>
        <?php else: ?>
          <a href="track-orders.php">Track Orders</a>
          <a href="cart.php">View Cart</a>
        <?php endif; ?>
      <?php else: ?>
        <a href="login.php">Sign In</a>
        <a href="register.php">Register</a>
      <?php endif; ?>
    </div>
    <p id="copyright">© 2026 PopCart. All rights reserved</p>
  </footer>

  <script src="index.js"></script>
</body>
</html>